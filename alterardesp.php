...
<section>
<article>
<h1>Alterar Despesas</h1>
<img src="icones/Alterar.png" />
<?php 
require 'conecta.php';
$id = $_GET["id"];
$result = pg_query($conn, "SELECT * FROM despesas WHERE id={$id}");
$row = pg_fetch_array($result)

?>
<form method="POST" action="<?php $PHP_SELF ?>" name="frmAlterarDesp">
<?php 
...
<?php
echo "	Data: <input type='date'  maxlength='20' size='11' name='data' value='".$row['data']."'  pattern='dd-mm-yyyy' /><br />";
echo "	Descrição:  <input type='text' maxlength='10' size='11' name='descricao' value='".$row['descricao']."'/><br />";
echo "	Despesa Fixa:  <input type='checkbox' name='fixo' value='".$row['fixo']."'/><br />";
?>
...
if (isset($_POST['descricao'])){
	$result = pg_query($conn, "UPDATE despesas SET valor='".$_POST['valor']."', tipo='".$_POST['tipo']."', data='".$_POST['data']."', descricao='".$_POST['descricao']."', fixo='".$_POST['fixo']."' WHERE id=".$_POST['id'].";");
	if (!$result) {
		echo "Ocorreu um erro!\n";
		exit;
	}
	else {echo "<script type='text/javascript'>alert('Cadastro Atualizado!');window.location='despesas.php';</script>";}
